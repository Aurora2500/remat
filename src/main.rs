...
#[tokio::main]
async fn main() {
	let (mut cam, mut buffers) = camera::Camera::new("/dev/video0").await.unwrap();
	...
	for fb in buffers.iter_mut() {
		cam.capture_frame(fb).await.expect("capturing failed");
	}

	let frame_data = cam
		.capture_frame(&mut buffers[0])
		.await
		.expect("capturing failed");
